...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	$password = hash('sha256', 'abcdefghijklmnopqrstuvwxyz1234567890' . $_POST['pass']);

	if ($_POST["uname"] == $_SESSION['email']) {
		$_SESSION["message"] = "You can't change email to same one.";
	} else {

		$sql = "SELECT * FROM members WHERE username='" . $_SESSION['email'] . "' AND password='$password'";
		$result = mysqli_query($con, $sql);
		$count = mysqli_num_rows($result);

		if ($count == 1) {
			$sql = "UPDATE members SET username='" . htmlspecialchars($_POST["uname"]) . "' WHERE username='" . $_SESSION['email'] . "' AND username='" . $password . "'";
			mysqli_query($con, $sql);
			$_SESSION["message"] = "Your email got updated to: " . $_POST["uname"] . "";
		} else {
			$_SESSION['error'] = "Invailid password, please try again.";
		}
	}
}
?>
...
<head>
	...
	<title>iLoot.it | Change Email</title>
			...
			<div class="row">
				<div class="col-md-8 col-md-offset-2 text-center colorlib-heading animate-box">
					...
					<form action="" method="post">
						<input type="text" placeholder="Enter New Email" name="uname" class="form-control" required></input>
						<input type="password" placeholder="Password" name="pass" class="form-control" required></input>
						<br>
