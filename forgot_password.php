<?php
	...
	if(isset($_POST['email'])){
		...
		$row=mysqli_fetch_array($con,$rs);
		$count=mysqli_num_rows($rs);


		if($count == 1){

			$_SESSION["message"] = "An email has been sent to the address you provided. Check your inbox or spam folder and click the link inside the email to continue the password recovery process.";
			
			$found = 0;
			while($found == 0) {
				$code = bin2hex(openssl_random_pseudo_bytes(8));

				$sql="SELECT * FROM core_verify WHERE code='".$code."'";
				$rs=mysqli_query($con,$sql);
				$count=mysqli_num_rows($rs);
				if($count == 0){

					$sql="INSERT INTO core_verify (code, type, email, time) VALUES ('".$code."', '2', '".$_POST['email']."', '".time()."')";
					if (mysqli_query($con, $sql)) {
						$found = 1;
					} else {
						$_SESSION["message"] = "";
						$_SESSION["error"] = "Something whent wrong, try again.";
					}
				} 
			}
...
// Subject
$subject = 'iLoot.it | Account recovery';
...
<html>
  <head>
    ...
    <title>iLoot.it | Account recovery</title>
    <style>
      /* -------------------------------------
          GLOBAL RESETS
      ------------------------------------- */
                ...
                <td class="wrapper">
                  <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                    <tr>
                      <td>
                        <p>Hello,</p>
                        <p>You recently requested to reset your password for your iLoot.it account. Please press the button below to reset your password.</p>
                            ...
                            <tr>
                              <td align="left">
                                <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                                  <tbody>
                                    <tr>
                                      <td> <a href="https://iloot.it/verify?code='.$code.'" target="_blank">Reset Password</a> </td>
                                    </tr>
                                  </tbody>
                                </table>
                              </td>
              ...
              <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  <td class="content-block">
                    <span class="apple-link">Copyright &copy; 2018-2019 MVDW Studios. All rights reserved.</span>
                  </td>
                </tr>
              </table>
            </div>
...
$headers[] = 'From: iLoot.it <noreply@iloot.it>';
...
mail($to, $subject, $message, implode("\r\n", $headers));
			...
			$_SESSION["error"] = "We're sorry. We could not find the email you provided. Please try again.";
		}
						...
						<div class="col-md-8 col-md-offset-2 text-center colorlib-heading animate-box">
							...
							<form action="/forgot_password" method="post">
								...
								<div class="row form-group">
									...
									<div class="col-md-4">
										<input type="submit" class="btn btn-primary" value="Send recovery email">
									</div>
								</div>
