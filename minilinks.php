<?php
	...
	if($status == "1"){
		...
		if(!empty($_POST['url'])) {
			...
			$sql = "SELECT * FROM skinnylinks WHERE url='" . $_POST['url'] ."' AND username='$username'";
			...
			if (filter_var($_POST['url'], FILTER_VALIDATE_URL) === FALSE) {
				...
				$_SESSION["error"] = "That's is not a valid url!";
				...
				while($found == 0){
					...
					if($count == 0){
						$found = 1;
						mysqli_query($con, "INSERT INTO `skinnylinks` (username, url, shorter, clicks) VALUES ('$username', '" . $_POST['url'] . "', '$shorter', '0')");
						$_SESSION["message"] = "https://iloot.it?m=$shorter";
				...
				// if (strpos($url, 'iloot.it/skinnylinks/') !== false) {
					// echo "You can't shorten an ilootit url.";

				// } else {
				// }

			} else {
				$_SESSION["error"] = "You can't shorten an url multiple types.";
			}
		...
		if(isset($_POST['h-captcha-response']) && !empty($_POST['h-captcha-response'])){
			...
			if($responseData->success){
				...
				if(!isset($_SESSION["link"])){
					...
					// $username = $_SESSION["email"];
					$sql = "SELECT * FROM skinnylinks WHERE shorter='" . $_SESSION['m'] . "'";
					$rs = mysqli_query($con,$sql);
					$row = mysqli_fetch_array($rs);
					$count = mysqli_num_rows($rs);
					...
					if ($count == 1) {
						...
						mysqli_query($con, "UPDATE skinnylinks SET clicks='" . $clicks . "' WHERE shorter='" . $_SESSION['m'] ."' AND username='" . $row['username'] . "'");
						...
						if($row['username'] == "leca.vladmihai@hotmail.com"){
							$pointsAdd = rand(20, 40);
						} else {
							$pointsAdd = rand(10, 20);
						}

						$points = $row1['points'] + $pointsAdd;
						$calc_time = time() - $row1['time'];



						$sql1 = "UPDATE members SET points='" . $points . "', time='" . time() . "' WHERE username='" . $row['username'] . "'";
						...
						$_SESSION["error"] = "It looks like the link you trying to visit got deleted.";
...
<html>
	<head>
		...
		<title><?php echo $name; ?> | MiniLinks</title>
	...
	<body style="visibility: hidden !important;">
		...
		<div id="page">
			...
			<div id="colorlib-contact">
				<div class="container">
					<div class="row">
						<div class="col-md-12 col-md-offset-0 text-center colorlib-heading animate-box">
							<h2>
								<?php
									if($status == "1"){
										echo "Minilinks";
							...
							<div class="row form-group">
								...
								<?php 
									if($status == "1"){
										...
										$username = $_SESSION["email"];
										$sql = "SELECT * FROM skinnylinks WHERE username='$username'";
										...
										echo "
											...
											<p>
												...
												<form action=\"/minilinks\" method=\"post\">
													<input type=\"text\" name=\"url\" class=\"form-control\" placeholder=\"Put here your URL\"><input class=\"btn btn-primary\" type=\"submit\" value=\"Shorten\">
												</form>
											</p>
										...
										echo "<tr><th>URL</th><th style=\"width: 400px\">Shorten</th><th>Clicks</th></tr>";

										while($row = $data->fetch_assoc()) {
											$shorten = "https://iloot.it?m=" . $row["shorter"];
											$url = $row["url"];
											$clicks = $row["clicks"];
											echo "<tr><td>$url</td><td style=\"width: 400px\">$shorten</td><td>$clicks</td></tr>";
										}
										...
										echo "
											<div>
												<p>We need to verify that you are not a robot, that's why we require you to complete a \"I am not a robot\" button.</p>
												<p>The person who sent you this URL makes a commission by us showing you advertisments.</p>
												<p>When you finish the human verification, please click on the \"Redirect Now!\" button, this will send you to the link intended for you.</p><br><br><b>Did you know you can make free money with this site!</b><br><a href=\"https://www.iloot.it\">Click here to earn</a>
											</div>
											<div>
												<form id=\"hcaptcha\" method=\"post\">
													<input type=\"hidden\" name=\"url\" value=\"" . $_SESSION['m'] . "\">
													<div class=\"h-captcha\" data-sitekey=\"fb530620-c446-4e98-a99e-f93d2a724456\"></div>
													<br />
													<div><input type=\"submit\" value=\"Redirect Now!\" id=\"r_verify\" class=\"btn btn-primary\"></div>
												</form>
											</div>
