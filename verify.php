 <?php
	...
	if($_POST['reset_password_submit'] != ""){
		$password=hash('sha256', 'hash'.$_POST["reset_password"]);
		$password_confirm=hash('sha256', 'hash'.$_POST["reset_password_confirm"]);
		...
		if($password == $password_confirm){
			$sql="SELECT * FROM core_verify WHERE code='".$_SESSION['code']."'";
			$rs=mysqli_query($con, $sql);
			$row=mysqli_fetch_array($rs);

			$changepwddate = date('d.m.Y G:i');

			$sql="UPDATE members SET password='".$password."', changepwddate='".$changepwddate."' WHERE username='".$row['email']."'";

			mysqli_query($con, $sql);
		$type = 2;
		$title = "Forgot password";
		$_SESSION['message'] = "Changed password.";
		} else {
	...
	if($type == 0){
		$sql="SELECT * FROM core_verify WHERE code='".$_GET['code']."'";
		$rs=mysqli_query($con, $sql);
		$row=mysqli_fetch_array($rs);
							...
							<form action="/verify" method="post">
								<div class="row form-group">
									<div class="col-md-12">
										<?php
											if($type == 0){
												echo "We can not verify your request, try again later.";
											}
										?>
								...
								<div class="row form-group">
									...
									<div class="col-md-12">
										<input type="password" name="reset_password_confirm" class="form-control" placeholder="Confirm new password">
									</div>
									<div class="col-md-12">
										<input type="submit" name="reset_password_submit" class="btn btn-primary" value="Reset password">
									</div>
